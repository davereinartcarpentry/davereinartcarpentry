...
Lightbox.prototype = {
	...
	initialize: function() {	
		...
		objLightboxImage.setAttribute('id','lightboxImage'); //added for print styles jds
		objLightboxImage.setAttribute('alt','No Image Loaded/Selected'); //added for print styles jds
		...
		var objBottomNavCloseClear = document.createElement("br");
		objBottomNavCloseClear.className = 'NavCloseClear';
		objBottomNav.appendChild(objBottomNavCloseClear);
	},
	...
	start: function(imageLink) {	
		Lightbox.dryrun = false;
		if(/Apple/.test(navigator.vendor)){
		var thislink = imageLink.href.replace(/[^-\w]/g, '_$_');
		Lightbox.donelinks = Lightbox.donelinks? Lightbox.donelinks : {};
			if(!Lightbox.donelinks[thislink]){
				Lightbox.donelinks[thislink] = 1;
				Lightbox.dryrun = imageLink;
				}
				}
		...
		new Effect.Appear('overlay', { duration: overlayDuration, to: overlayOpacity });
		...
		onscroll=function(){
		var lightboxLeft = document.documentElement.scrollLeft;
		Element.setLeft('lightbox', lightboxLeft);
		}
	...
	changeImage: function(imageNum) {	
		...
		$('lightboxImage').removeAttribute('width', 0);
		$('lightboxImage').removeAttribute('height', 0);
...
/*mark1*/			$('lightboxImage').style.visibity='hidden';
			...
			myLightbox.start(Lightbox.dryrun);
			return;
			}
			$('lightboxImage').style.display=tempd;
			$('lightboxImage').style.visibity='visible';
